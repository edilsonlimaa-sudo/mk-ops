...
import { formatCoordinatesForDisplay } from "@/utils/formatCoordinatesDisplay";
...
/** `true` = texto cru + layout antigo (só para comparar). Volte para `false` quando terminar. */
const LEGACY_COORDENADAS_ROW = false;
          ...
          >
            {cliente.endereco && (
              <EditableInfoRow
                ...
                onEdit={() =>
                  openEditModal("endereco", cliente.endereco || "", "Endereço")
                }
              />
            )}
            {cliente.numero && (
              <EditableInfoRow
                ...
                onEdit={() =>
                  openEditModal("numero", cliente.numero || "", "Número")
                }
              />
            )}
            {cliente.complemento && (
            ...
            {cliente.bairro && (
              <EditableInfoRow
                ...
                onEdit={() =>
                  openEditModal("bairro", cliente.bairro || "", "Bairro")
                }
              />
            )}
            {cliente.cidade && (
              <EditableInfoRow
                ...
                onEdit={() =>
                  openEditModal("cidade", cliente.cidade || "", "Cidade")
                }
              />
            )}
            {cliente.estado && (
              <EditableInfoRow
                ...
                onEdit={() =>
                  openEditModal("estado", cliente.estado || "", "Estado")
                }
              />
            )}
            {cliente.cep && (
              <EditableInfoRow
                label="CEP"
                value={cliente.cep}
                onEdit={() => openEditModal("cep", cliente.cep || "", "CEP")}
              />
            )}
            {cliente.coordenadas &&
              cliente.coordenadas !== "-38.5748,-3.741162,0" &&
              (LEGACY_COORDENADAS_ROW ? (
                <TouchableOpacity
                ...
                <TouchableOpacity
                  onPress={() => setMapModalVisible(true)}
                  className="flex-row items-center justify-between gap-2 py-3 px-3 -mx-3 rounded-lg bg-blue-50 border border-blue-100 active:bg-blue-100"
                >
                  ...
                  <View className="flex-row items-center gap-2 flex-1 min-w-0 justify-end">
                    <Text
                      className="text-gray-900 text-sm font-medium text-right flex-1 min-w-0"
                      numberOfLines={1}
                      ellipsizeMode="tail"
                    >
                      {formatCoordinatesForDisplay(cliente.coordenadas)}
                    </Text>
                    <Ionicons name="map" size={18} color="#3b82f6" />
                  </View>
                </TouchableOpacity>
